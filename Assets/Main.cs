...
public class Main : MonoBehaviour
{
    ...
    public float interTrialInterStimulusDuration = 0.1f; // Inter-trial interval and inter-stimulus interval in seconds
    public float imageDisplayDuration = 0.1f; // Time images are displayed in seconds
    public float subjectBreakDuration = 1f; // Length of subject break when 1/3 and 2/3 done in seconds
    ...
    {      
        ...
        if (ranOrder == 1)
        {
            ...
            leftTex = (Texture2D)Resources.Load("Conditions/" + condition + "/luminance-0_4000-reflectance-" + standardImageOrderNumber[iteration-1] + "-BigBall-Library", typeof(Texture2D));
            rightTex = (Texture2D)Resources.Load("Conditions/" + condition + "/luminance-0_4000-reflectance-" + standardImageOrderNumber[iteration-1] + "-BigBall-Library_camera2", typeof(Texture2D));
            j--;
            ranOrder = 0;
        } // if
        else
        {
            ...
            // Finds the textures for the left and right images
            leftTex = (Texture2D)Resources.Load("Conditions/" + condition + "/luminance-0_" + comparisonImageOrder_LightnessName[iteration-1] + "-reflectance-" + comparisonImageOrder_ImageNumber[iteration-1] + "-BigBall-Library", typeof(Texture2D));
            rightTex = (Texture2D)Resources.Load("Conditions/" + condition + "/luminance-0_" + comparisonImageOrder_LightnessName[iteration-1] + "-reflectance-" + comparisonImageOrder_ImageNumber[iteration-1] + "-BigBall-Library_camera2", typeof(Texture2D));
            j--;
            ranOrder = 1;
        } // else

        yield return new WaitForSecondsRealtime(interTrialInterStimulusDuration);

        // Finds the left object and applies left texture
    ...
    {
        yield return new WaitForSecondsRealtime(subjectBreakDuration);
